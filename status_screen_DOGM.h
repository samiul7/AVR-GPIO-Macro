...
FORCE_INLINE void _draw_centered_distance(const int16_t distance, const uint8_t x, const uint8_t y) {
  const char * const str = itostr3(distance);
  u8g.setPrintPos(x - (str[0] != ' ' ? 0 : str[1] != ' ' ? 1 : 2) * DOG_CHAR_WIDTH / 2, y);
  lcd_print(str);
}
...
static void lcd_implementation_status_screen() {
  ...
  if(pouchHeater1_on)
    u8g.drawBox(57,17,2,2);

  // pouch heater 1
  _draw_centered_temp(pouchHeater1_set_temp, 46, 7);
  _draw_centered_temp(pouchHeater1_current_temp, 46, 28); 

  if(pouchHeater2_on)
    u8g.drawBox(79,17,2,2);

  // pouch heater 2
  _draw_centered_temp(pouchHeater2_set_temp, 68, 7);
  _draw_centered_temp(pouchHeater2_current_temp, 68, 28);

  //draw pump1 rotating circle
  if((pump1_On || manuallyPump1_On) && (sensor1_current_distance >= (uint16_t) sensorThreshold))
    u8g.drawCircle(97, 3, 3);

  // sensor limit
  _draw_centered_distance(sensorThreshold, 100, 7);

  //draw pump2 rotating circle
  if((pump2_On || manuallyPump2_On) && (sensor2_current_distance >= (uint16_t) sensorThreshold))
    u8g.drawCircle(119, 3, 3);


  // VL6180X sensor distance
  _draw_centered_distance(sensor1_current_distance,  89, 28); 
  _draw_centered_distance(sensor2_current_distance, 109, 28);
  ...
  // u8g.print(distance(NOZZLE_1_SENSOR));

  // u8g.setPrintPos(110, 28);
  // u8g.print(distance(NOZZLE_2_SENSOR));
  //--------------------------------------------------------------
