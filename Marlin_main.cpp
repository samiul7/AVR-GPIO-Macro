...
extern uint32_t extruderSteps; //added by samiul, for pump enable 
/**
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      // code added by samiul
      //----------------- get pouch heater temparature from pc-------------------------
      case 9: //M9 P1 T50 [M_code P_whichPouchHeater T_temp]
        if(parser.seen('P'))
        {
          bool waitForHeating= false;
          do
          {
            switch (parser.value_int())
            {
              case 1: //pouch heater 1 
                if(parser.seen('T'))
                {
                  waitForHeating = true;
                  pouchHeater1_on = true;
                  pouchHeater1_set_temp = parser.value_int();
                  SERIAL_ECHOLNPGM("Pouch 1 heating...");
                  isHeating = true;
                }
              break;
            
              case 2: //pouch heater 2
                if(parser.seen('T'))
                {
                  waitForHeating = true;
                  pouchHeater2_on = true;
                  pouchHeater2_set_temp = parser.value_int();
                  SERIAL_ECHOLNPGM("Pouch 2 heating...");
                  isHeating = true;
                }
              break;
            } 
            ...
            reset_stepper_timeout(); // Keep steppers powered
            manage_pouchHeater();
          } while(waitForHeating && isHeating);
          SERIAL_ECHOLNPGM("Heating done");
        }
      break; 

      case 10: //M10 P1 [M_code P_whichPouchHeater]
        if(parser.seen('P'))
        {
          switch (parser.value_int())
          {
            case 1: //pouch heater 1 
              pouchHeater1_on = false;
              SERIAL_ECHOLNPGM("Pouch 1 Heater off");
            break;
          
            case 2: //pouch heater 2
              pouchHeater2_on = false;
              SERIAL_ECHOLNPGM("Pouch 2 Heater off");
            break;
          }  
        }
      break;
...
void disable_e_stepper(const uint8_t e) {
  //If the nozzle stepper is disabled, the pump will be disabled.
  //added by samiul for pump control
  //when the extruder motor is turned off, the pump is also turned off at the same time
  pump1_On = false;
  pump2_On = false;
...
void setup() {
  ...
  //no need servo edited by samiul
...
/*-----------------------This code added for peristaltic pump enable---------------------*/
//find which extruder is active (this code will help us to to enable pump)
  if (extruderSteps > 0)
  {
    switch (active_extruder)
  	{
    ...
    // SERIAL_ECHOPAIR("Extruder Steps:", extruderSteps);
    // SERIAL_ECHOLNPGM(" steps");
  }
  /*------------------------------------added by samiul-----------------------------------*/
  //SERIAL_ECHOPAIR("Sensor Distance:", (uint16_t)distance());

  //----------------manage pump and heater------------------------
  manage_pump();
  manage_pouchHeater();
  //-------------------added by samiul----------------------------
