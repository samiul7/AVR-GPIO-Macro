...
uint16_t max_display_update_time = 0;
  ...
  void lcd_control_pump_menu();
  void lcd_control_sensor_menu();
  void lcd_control_sensorInit_menu();
...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  void lcd_prepare_menu() {
    ...
    //----------------------Change filament does not need for 3d food printer-------------------
    /*
    // Change filament
  ...
  #if E_MANUAL > 1
    void lcd_move_e0() { active_extruder= 0; _lcd_move_e(0); } //added by samiul "active_extruder = 0;"
    void lcd_move_e1() { active_extruder= 1; _lcd_move_e(1); } //added by samiul "active_extruder = 1;"
    #if E_MANUAL > 2
  ...
  void lcd_move_menu() {
    ...
    if (_MOVE_XYZ_ALLOWED) {
      ...
      //MENU_ITEM(submenu, MSG_MOVE_E, lcd_move_get_e_amount);  //commented by samiul
      #if E_MANUAL > 1
        MENU_ITEM(submenu, "Nozzle 1", lcd_move_get_e0_amount); // Added by samiul "Nozzle 1"
        MENU_ITEM(submenu, "Nozzle 2", lcd_move_get_e1_amount); // Added by samiul "Nozzle 2"
        #if E_MANUAL > 2
          MENU_ITEM(submenu, MSG_MOVE_E MSG_MOVE_E3, lcd_move_get_e2_amount);
          #if E_MANUAL > 3
  ...
  void lcd_control_menu() {
    ...
    MENU_ITEM(submenu, "Pump", lcd_control_pump_menu); //added by samiul
    MENU_ITEM(submenu, "Sensor", lcd_control_sensor_menu); //added by samiul

    //----------------no need filament setting for a food printer-------------- 
    /*   
    #if DISABLED(NO_VOLUMETRICS) || ENABLED(ADVANCED_PAUSE_FEATURE)
  ...
  void lcd_control_temperature_menu() {
    ...
    //-----------------------added by samiul-------------------------
    MENU_ITEM_EDIT(bool, "Pouch 1 heating", &pouchHeater1_on);
    MENU_ITEM_EDIT(bool, "Pouch 2 heating", &pouchHeater2_on);

    MENU_MULTIPLIER_ITEM_EDIT(int3, "Pouch 1", &pouchHeater1_set_temp, 25, 100);
    MENU_MULTIPLIER_ITEM_EDIT(int3, "Pouch 2", &pouchHeater2_set_temp, 25, 100);
    //---------------------------------------------------------------
    ...
    //-------------------fan speed does not need for 3d food printer---------------------------
    /*
    #if FAN_COUNT > 0
      #if HAS_FAN0
        MENU_MULTIPLIER_ITEM_EDIT(int3, MSG_FAN_SPEED FAN_SPEED_1_SUFFIX, &fanSpeeds[0], 0, 255);
    ...
    */
    ...
    //-------------------Autotemp does not for 3d food printer---------------------------
    /*
    //
    ...
    */
    ...
    //-------------------Preheat Material does not need for 3d food printer-----------------------
    /*
    #if DISABLED(SLIM_LCD_MENUS)
      //
  ...
  */
  void lcd_control_pump_menu() {
    ...
    MENU_ITEM_EDIT(bool, "Pump 1", &manuallyPump1_On);
    MENU_ITEM_EDIT(bool, "Pump 2", &manuallyPump2_On);

    MENU_MULTIPLIER_ITEM_EDIT(int3, "Pump 1 Speed[%]", &pump1Speed, 0, 100);
    MENU_MULTIPLIER_ITEM_EDIT(int3, "Pump 2 Speed[%]", &pump2Speed, 0, 100);
   ...
   *
   *
   * "Control" > "Sensor" submenu
   *
  */
  void lcd_control_sensor_menu() {
    START_MENU();
    MENU_BACK(MSG_CONTROL);
    MENU_MULTIPLIER_ITEM_EDIT(int3, "Sensor limit[mm]", &sensorThreshold, 10, 80);
  ...
  // void lcd_control_sensorInit_menu() {  
  //   VL6180X_manuallyInit(); //added by samiul
  //   lcd_completion_feedback();
  // }
